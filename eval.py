...
from utils.make_wav_id_dict import make_wav_id_dict

parser = argparse.ArgumentParser()
parser.add_argument(
    "--test_dir", type=str, default="~/DNS-Challenge-master/datasets/no_reverb",required=False, help="Test directory including wav files"
)
parser.add_argument(
...
def main(conf):
    ...
    conf["test_dir"] = os.path.expanduser(conf["test_dir"])
    dict_list = get_wavs_dict_list(conf["test_dir"])
    save_dir = os.path.join(conf["exp_dir"], "eval_examples/")
    ...
    filename = os.path.join(conf["exp_dir"], "final_metrics.json")
...
def evaluate(dict_list, model, conf, save_dir=None):
    ...
    for idx, wav_dic in enumerate(tqdm(dict_list)):
        ...
        with torch.no_grad():
            ...
            est_clean_np = model.forward(net_input).squeeze().cpu().data.numpy()
